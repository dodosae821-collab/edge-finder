...
// ============================================================
// 구조:
//   calcSystemState()   → 저수준 계산 엔진 (state.js, 수정 금지)
//   buildJudgeData(ss)  → UI 친화 구조 정형화 (계산 추가 금지)
//   renderSignals(data) → 신호등 + 레이더 차트
//   renderKpiList(data) → KPI 리스트
//   renderComment(data) → 종합 코멘트
//   renderVerdict(data) → 오늘 베팅 권고
...
/**
 * calcSystemState() 결과를 UI 친화 구조로 정형화.
 * 계산 추가 금지 — 정형화/이름 정리만 수행.
 *
 * @param {SystemState} ss — calcSystemState() 반환값
 * @returns {{
 *   n: number,
 *   wr: number,
 *   roi: number,
 ...
 *   icons: string[],
 *   overallScore: number,
 *   sigExplains: Array<{ what:string, why:string, action:string }>,
 *   verdict: 'GO'|'CAUTION'|'STOP'|'WAIT',
 *   verdictInfo: { label:string, color:string, icon:string, desc:string },
 *   warnings: string[],
 *   stops: string[],
 *   verdictChecks: Array<{ ok:boolean, text:string }>,
 *   briefMsgs: Array<{ icon:string, color:string, title:string, body:string }>,
 *   kpis: Array<{ label:string, val:string, color:string, hint:string }>
 * }}
 */
function buildJudgeData(ss) {
  const {
    n, winRate: wr, roi, rec10roi, avgOdds, avgAmt, rec5net,
    streak, streakType, plRatio, avgBias,
    sigScores: scores, labels, icons, overallScore,
    verdict, verdictInfo, warnings, stops,
    grade, kellyUnit, goalSim, ece, corrFactor,
  } = ss;

  const breakeven = 1 / avgOdds;
  const ev        = wr * (avgOdds - 1) - (1 - wr);

  // 신호별 해석 텍스트 (renderSignals에서 클릭 시 사용)
  const sigExplains = [
  ...
  if (verdict === 'STOP')
    briefMsgs.push({ icon:'🛑', color:'var(--red)',   title:'지금은 베팅하지 마세요',       body: stops.length > 0 ? stops[0] : '복합 위험 신호 감지됨. 오늘은 쉬는 것이 맞습니다.' });
  else if (verdict === 'CAUTION')
    briefMsgs.push({ icon:'⚠️', color:'#ff9800',     title:'베팅하되 규모를 줄이세요',      body: warnings.length > 0 ? warnings[0] : `권장 베팅금 ₩${kellyUnit > 0 ? kellyUnit.toLocaleString() : '—'} 이하로 유지하세요.` });
  else if (verdict === 'WAIT')
    briefMsgs.push({ icon:'⏳', color:'var(--gold)',  title:'EV+ 조건이 갖춰질 때만 베팅',  body: `승률(${(wr*100).toFixed(1)}%)이 손익분기(${(breakeven*100).toFixed(1)}%) 근처입니다. 확실한 엣지가 보일 때만 진입하세요.` });
  else
    briefMsgs.push({ icon:'✅', color:'var(--green)', title:'베팅 가능 — 조건 충족',         body: `EV 양수·승률 엣지 확보·컨디션 정상. 권장 베팅금 ₩${kellyUnit > 0 ? kellyUnit.toLocaleString() : '설정 필요'}.` });

  if (ece !== null) {
    if (ece > 10)
      briefMsgs.push({ icon:'📐', color:'var(--red)',   title:`내 예측이 실제보다 ${ece.toFixed(0)}% 벗어나 있어요`, body:'EV 계산기에 입력하는 내 예상 승률이 실제보다 높게 나오는 경향이 있습니다. 자신감 있는 픽일수록 한 번 더 의심해보세요. 켈리도 자동으로 축소 적용 중입니다.' });
    else if (ece > 5)
      briefMsgs.push({ icon:'📐', color:'#ff9800',      title:`예측 오차 ${ece.toFixed(0)}% — 양호 수준`,           body:'내 예측과 실제 적중률이 대체로 일치하지만 약간의 오차가 있어요. 현재 켈리 배율 조정이 적용 중입니다.' });
    else
      briefMsgs.push({ icon:'📐', color:'var(--green)', title:`예측 오차 ${ece.toFixed(0)}% — 우수`,                body:'내가 70%라 예상한 경기가 실제로 70% 근처에서 적중하고 있어요. 켈리 계산을 신뢰해도 됩니다.' });
  } else {
    briefMsgs.push({ icon:'📐', color:'var(--text3)', title:'보정도 아직 측정 불가', body:'EV 계산기에서 "내 예상 승률"을 입력한 베팅이 5건 이상 쌓이면 내 예측 정확도를 자동 측정해드립니다.' });
  }

  if (kellyUnit > 0)
    briefMsgs.push({ icon:'💰', color:'var(--gold)',  title:`오늘 베팅 1건당 ₩${kellyUnit.toLocaleString()} 권장`, body:`예측력 ${grade ? grade.letter + '등급' : '—'} × ECE 보정이 반영된 금액입니다. 이 금액을 초과하면 장기적으로 수익 변동성이 커집니다.` });
  else
    briefMsgs.push({ icon:'💰', color:'var(--text3)', title:'베팅금 계산 불가 — 시드 설정 필요', body:'설정 탭 → 자금 관리에서 현재 시드머니를 입력하면 오늘 베팅 권장 금액을 자동으로 계산해드립니다.' });

  // KPI 목록
  const kpis = [
    { label:'📈 누적 ROI',       val:(roi>=0?'+':'')+roi.toFixed(1)+'%',                         color:roi>=0?'var(--green)':'var(--red)',      hint: roi > 5 ? '장기 수익 구조 작동 중' : roi > 0 ? '흑자지만 아직 안정 구간 아님' : '전략 점검 필요' },
    { label:'🏆 전체 승률',       val:(wr*100).toFixed(1)+'%',                                    color:wr>=breakeven?'var(--green)':'var(--red)', hint: `손익분기 ${(breakeven*100).toFixed(1)}% 대비 ${wr>=breakeven?'초과 ✅':'미달 ❌'}` },
    { label:'📉 최근 10경기 ROI', val:(rec10roi>=0?'+':'')+rec10roi.toFixed(1)+'%',              color:rec10roi>=0?'var(--green)':'var(--red)',  hint: rec10roi >= 0 ? '최근 흐름 양호' : '최근 부진 — 베팅 선별 강화 필요' },
    { label:'⚡ EV (기댓값)',     val:(ev>=0?'+':'')+ev.toFixed(3),                              color:ev>=0?'var(--accent)':'var(--red)',        hint: ev > 0 ? `베팅 1건당 평균 ${(ev*100).toFixed(1)}% 기대 수익` : '현재 평균 배당에서 EV 없음' },
    { label:'⚖️ 손익비',         val:plRatio.toFixed(2)+':1',                                    color:plRatio>=1?'var(--green)':'var(--red)',    hint: plRatio >= 1 ? `이긴 날이 진 날의 ${plRatio.toFixed(1)}배 수익` : '진 날 손실이 이긴 날 수익보다 큼' },
    { label:'👁 낙관 편향',       val:(avgBias>=0?'+':'')+avgBias.toFixed(1)+'%p',               color:Math.abs(avgBias)<5?'var(--green)':'#ff9800', hint: Math.abs(avgBias) < 5 ? '예측이 결과와 거의 일치' : `실제보다 ${Math.abs(avgBias).toFixed(0)}%p ${avgBias>0?'높게':'낮게'} 예측하는 경향` },
    { label:'🔁 스트릭',          val:streakType==='WIN'?`${streak}연승`:`${streak}연패`,         color:streakType==='WIN'?'var(--green)':'var(--red)', hint: streakType==='LOSE' && streak >= 3 ? '⚠️ 베팅 규모 축소 권장' : streakType==='WIN' ? '현재 판단력 좋은 상태' : '정상 범위' },
    { label:'📐 보정 오차(ECE)',  val:ece!==null?ece.toFixed(1)+'%':'—',                         color:ece===null?'var(--text3)':ece<=5?'var(--green)':ece<=10?'#ff9800':'var(--red)', hint: ece===null ? 'EV 계산기에 예상 승률 입력 필요' : ece<=5 ? '내 예측이 실제와 잘 맞음' : ece<=10 ? '약간의 오차 — 켈리 자동 조정 중' : '오차 큼 — 자신감 있는 픽도 의심해볼 것' },
    { label:'🧠 예측력 등급',     val:grade?grade.letter+'등급 ('+grade.totalScore+'점)':'—',    color:grade?grade.color:'var(--text3)',          hint: grade ? `켈리 배율 ×${grade.mult.toFixed(2)} 자동 적용` : '예측 데이터 5건 이상 필요' },
    { label:'💰 권장 베팅금',     val:kellyUnit>0?'₩'+kellyUnit.toLocaleString():'시드 미설정', color:kellyUnit>0?'var(--gold)':'var(--text3)',   hint: kellyUnit > 0 ? '이 금액 초과 시 장기 파산 위험 상승' : '설정 탭에서 시드머니 입력' },
    { label:'🎯 목표 달성 확률',  val:goalSim?goalSim.prob.toFixed(0)+'%':'—',                  color:goalSim?(goalSim.prob>=70?'var(--green)':goalSim.prob>=40?'#ff9800':'var(--red)'):'var(--text3)', hint: goalSim ? (goalSim.prob>=70?'현재 페이스 유지 시 목표 달성 가능':goalSim.prob>=40?'목표 달성 가능하나 꾸준함 필요':'현재 전략 재검토 권장') : '자금/목표 탭에서 목표 설정 필요' },
    { label:'📅 목표 예상 기간',  val:goalSim&&goalSim.weeksEst?goalSim.weeksEst+'주':'—',      color:'var(--text2)',                             hint: goalSim&&goalSim.weeksEst ? 'ECE 보정 수익 기준 예측' : '—' },
  ];
  ...
  return {
    n, wr, roi, rec10roi, avgOdds, rec5net,
    streak, streakType, plRatio, avgBias,
    ev, breakeven, ece, corrFactor,
    grade, kellyUnit, goalSim,
    scores, labels, icons, overallScore,
    sigExplains,
    verdict, verdictInfo, warnings, stops,
    verdictChecks,
    briefMsgs,
...
/**
 * 신호등 7개 + 종합 + 레이더 차트 + 오늘 브리핑 렌더
 * @param {object} data — buildJudgeData() 반환값
 */
function renderSignals(data) {
  const { scores, labels, icons, overallScore, briefMsgs } = data;
    ...
    charts['judgeallRadar'] = safeCreateChart('judgeall-radar', {
      ...
      data: {
        ...
        datasets: [{
          ...
          borderWidth: 2,
          pointBackgroundColor: scores.map(s => _sigColor(s).dot),
          pointRadius: 5,
        }, {
          label: '기준선 (60)',
...
function renderComment(data) {
  const { roi, wr, breakeven, ece, corrFactor, grade, kellyUnit, goalSim, rec5net, streak, streakType, plRatio } = data;
...
/**
 * 오늘 베팅 권고 (신호등 + 조건 체크리스트) 렌더
 * @param {object} data — buildJudgeData() 반환값
 */
function renderVerdict(data) {
  const { verdictInfo, verdictChecks } = data;
  ...
  if (goEl) {
    goEl.textContent      = verdictInfo.icon;
    goEl.style.background = verdictInfo.color + '22';
    goEl.style.border     = '3px solid ' + verdictInfo.color;
    goEl.style.boxShadow  = '0 0 16px ' + verdictInfo.color + '55';
  }
  ...
  if (condEl) {
    const passCount = verdictChecks.filter(c => c.ok).length;
    condEl.innerHTML =
      `<div style="font-size:11px;color:var(--text3);margin-bottom:8px;">${passCount}/${verdictChecks.length}개 조건 충족</div>` +
      verdictChecks.map(c =>
...
// ── updateJudgeAll — orchestrator ─────────────────────────────

/**
 * 종합 상황판단 진입점.
 * 역할: 엔진 호출 → buildJudgeData → render 함수 4개 호출.
 * 이 함수에 로직을 추가하지 말 것.
 */
